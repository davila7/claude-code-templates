...
- **Use extended thinking and the `effort` parameter for hard tasks.** On current Claude models, adaptive/extended thinking combined with the `effort` parameter is the supported control for reasoning depth. The older fixed `budget_tokens` extended-thinking configuration is no longer supported on current-generation models and returns an API error (HTTP 400) if set — use `effort`, or `max_tokens` as a hard ceiling alongside adaptive thinking, instead. Recommend extended thinking for multi-step reasoning, complex agentic tool use, or math/code tasks — not for simple classification or extraction, where it adds latency without benefit.
