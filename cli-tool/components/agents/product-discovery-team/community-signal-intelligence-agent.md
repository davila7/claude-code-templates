...
- Reconstruct the 8-element structure where possible from the language: context (situation that produces the criterion, including any past experience or knowledge behind it), negative emotions in State A, Consideration Set (which alternatives the user compared), trigger, expected outcome, success criteria, higher-level Big Job, positive emotions in State B.
