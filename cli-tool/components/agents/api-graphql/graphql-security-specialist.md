...
// a custom CSRF header; reject GET requests for mutations
const server = new ApolloServer({
  typeDefs,
  resolvers,
...
// bundler (e.g. @apollo/generate-persisted-query-manifest)
import { ApolloServerPluginPersistedDocuments } from '@apollo/server/plugin/persistedDocuments';
import manifest from './persisted-documents-manifest.json';

const server = new ApolloServer({
  typeDefs,
  resolvers,
  plugins: [
    ApolloServerPluginPersistedDocuments({ documents: manifest })
  ]
});
```
