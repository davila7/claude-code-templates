...
- Populate the new ADR's `References` section with any related ADRs found (link using relative paths, e.g. `docs/adr/adr-0003-monolith-first.md`)
- If this decision **supersedes** an existing ADR:
  - Set `supersedes: "adr-NNNN"` in the new ADR's front matter
  - Use `Edit` to update the old ADR's front matter: set `status: "Superseded"` and `superseded_by: "adr-MMMM"` (the new ADR's ID)
