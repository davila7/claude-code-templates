...
**Python and TypeScript use a multi-file layout** - directory `{lang}/claude-api/` containing `README.md` (install, client init, basic request, thinking, caching, stop details, misc), `tool-use.md` (tool definitions, agentic loop, Anthropic-defined tools, structured outputs), `streaming.md`, `batches.md`, `files-api.md`. **C#, Go, Java, PHP, and Ruby consolidate the same sections into a single file**, `{lang}/claude-api.md`, using the same section headings (`## Tool Use`, `## Streaming`, `## Files API`, `## Message Batches API`, etc.) - wherever this document says `{lang}/claude-api/README.md`, `{lang}/claude-api/tool-use.md`, `{lang}/claude-api/streaming.md`, `{lang}/claude-api/batches.md`, or `{lang}/claude-api/files-api.md` for one of those five languages, read the matching section of `{lang}/claude-api.md` instead. Not every language has every section (e.g., Ruby has no Batches section); if a section is absent, that feature's example is not yet documented for that language - fall back to the cURL shape or WebFetch the SDK repo from `shared/live-sources.md`. **cURL** -> `curl/examples.md`.

The Quick Task Reference below uses the `{lang}/claude-api/FILE.md` path notation for all languages; substitute `{lang}/claude-api.md` (and read the named section) for C#, Go, Java, PHP, and Ruby as described above.
