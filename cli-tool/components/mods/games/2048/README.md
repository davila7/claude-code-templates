...
2048 above the Claude Code prompt, for the minutes Claude spends working: slide and merge tiles up to 2048. The game pauses and its status line says so when Claude finishes the turn, so you never miss a reply. Playing costs no tokens: the mod answers every key and click itself.
