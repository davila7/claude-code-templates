...
const gatewayAnswer = (
  ...
  JSON.stringify({
    answers: {
      ...
      // As the API sends it: probabilities keyed "0".."4" by level, the score
      // their weighted mean; here nearly all of it on the nearest level.
      effort: { type: 'score', score: effort, probabilities: { [String(Math.round(effort))]: 0.9 } },
...
test('the rubric score maps onto the reasoning ladder', () => {
  ...
  expect(effortLevel(4)).toBe('max')
  expect(effortLevel(99)).toBe('max')
})
...
test('OpenRouter speaks TypeSafe\'s wire shape at its System One route', () => {
  const url = endpoint('openrouter', DEFAULT_BASE_URL.openrouter)
  expect(url).toBe('https://openrouter.ai/api/v1/systemone')
  expect(endpoint('openrouter', 'https://openrouter.ai/api/')).toBe(url)
  expect(DEFAULT_MODEL.openrouter).toBe('~typesafe/jev-latest')

  const body = JSON.parse(requestBody('openrouter', { prompt: 'x' }, DEFAULT_MODEL.openrouter) as string)
  expect(body.model).toBe('~typesafe/jev-latest')
  expect(body.state).toEqual({ prompt: 'x' })
  // Every question goes as TypeSafe asks it: the yes/no one as a noul.
  expect(body.questions).toEqual(questions('typesafe'))
  expect(body.questions.risky.type).toBe('noul')

  const headers = requestHeaders('openrouter', 'or-key', DEFAULT_MODEL.openrouter)
  expect(headers.authorization).toBe('Bearer or-key')
  expect(headers['ai-model-id']).toBeUndefined()
  expect(headers['ai-gateway-protocol-version']).toBeUndefined()
})

test('an OpenRouter answer, with its type tags and usage, reads like a TypeSafe one', () => {
  const answer = JSON.stringify({
    id: 'gen-1',
    model: 'typesafe/jev-1.13',
    provider: 'TypeSafe',
    answers: {
      tier: { type: 'choice', choice: 'fast', confidence: 0.93, probabilities: { fast: 0.93, balanced: 0.05, deep: 0.02 } },
      effort: { type: 'score', score: 0.4, confidence: 0.81 },
      risky: { type: 'noul', noul: 0.02 },
    },
    usage: { inputTokens: 120, outputTokens: 0, cost: 0.0001 },
  })
  ...
  expect(decision).toEqual({
    ...
    confidence: 0.93,
    effort: 0.4,
    effortConfidence: 0.81,
    effortProbabilities: null,
    risky: 0.02,
    strategy: null,
    strategyConfidence: null,
  })
  // Its confidence is a calibrated one, so it may clear the downgrade bar.
  expect(route(decision, on('claude-sonnet-5'), config).model).toBe('haiku')
})

test('auto takes TypeSafe, then OpenRouter, then the Gateway', () => {
  ...
  expect(selectProvider('auto', '', '', 'or')).toBe('openrouter')
})

test('a forced OpenRouter needs its own key, and no other backend takes it', () => {
  expect(selectProvider('openrouter', 'ts', 'gw', '')).toBeNull()
  expect(selectProvider('openrouter', '', '', 'or')).toBe('openrouter')
  expect(selectProvider('typesafe', '', '', 'or')).toBeNull()
  expect(selectProvider('gateway', '', '', 'or')).toBeNull()
  expect(selectProvider('builtin', 'ts', 'gw', 'or')).toBeNull()
})
...
// become an id there; the engine refuses "haiku" on a request. The id is the
// one the engine was seen using, never one written in the router.
test('a family alias resolves to the id the engine was seen using', () => {
  const ids = modelIds()
  for (const id of ['claude-haiku-4-5-20251001', 'claude-sonnet-5', 'claude-opus-5-5[1m]']) ids.learn(id)
  expect(requestModelId('haiku', ids)).toBe('claude-haiku-4-5-20251001')
  expect(requestModelId('Sonnet', ids)).toBe('claude-sonnet-5')
  expect(requestModelId('opus', ids)).toBe('claude-opus-5-5[1m]')
})

test('a family not seen yet has no id, so the main loop is not switched', () => {
  const ids = modelIds()
  ids.learn('claude-opus-5-5')
  expect(requestModelId('haiku', ids)).toBeNull()
  // An alias reported as a model teaches nothing.
  ids.learn('sonnet')
  expect(requestModelId('sonnet', ids)).toBeNull()
})

test('the newest release of a family wins, whatever order they are seen in', () => {
  const ids = modelIds()
  for (const id of ['claude-opus-5', 'claude-opus-5-5', 'claude-opus-4-1', 'claude-opus-6']) ids.learn(id)
  expect(ids.idFor('opus')).toBe('claude-opus-6')
  // A date is not a minor version: 4-5 dated still loses to 5.
  const haiku = modelIds()
  haiku.learn('claude-haiku-5')
  haiku.learn('claude-haiku-4-5-20251001')
  expect(haiku.idFor('haiku')).toBe('claude-haiku-5')
  // An id of another shape is kept only until a versioned one arrives.
  const odd = modelIds()
  odd.learn('us.anthropic.sonnet-custom')
  expect(odd.idFor('sonnet')).toBe('us.anthropic.sonnet-custom')
  odd.learn('claude-sonnet-5')
  expect(odd.idFor('sonnet')).toBe('claude-sonnet-5')
})

test('a full id or an unknown name is passed through unchanged', () => {
  const ids = modelIds()
  expect(requestModelId('claude-haiku-4-5-20251001', ids)).toBe('claude-haiku-4-5-20251001')
  expect(requestModelId('claude-opus-5[1m]', ids)).toBe('claude-opus-5[1m]')
  expect(requestModelId('my-org/custom', ids)).toBe('my-org/custom')
})
