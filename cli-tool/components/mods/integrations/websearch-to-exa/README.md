...
It is written to `.claude/skills/websearch-to-exa/`, which Claude Code auto-loads as `websearch-to-exa@skills-dir`. For one session with hot reload: `claude --plugin-dir .claude/skills/websearch-to-exa`. `claude plugin validate .claude/skills/websearch-to-exa` prints every event it hooks and every `$` call it makes.
