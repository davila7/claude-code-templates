          ...
          <div className="flex items-center gap-2">
            ...
            {isMarkdown && (
              <button onClick={() => { setSearchOpen(!searchOpen); if (!searchOpen) setTimeout(() => searchInputRef.current?.focus(), 50); }}
                className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs transition-colors ${searchOpen ? 'bg-accent-500/15 text-accent-400' : 'text-text-tertiary hover:text-text-secondary hover:bg-glass'}`}
              ...
              <div className="absolute z-20 top-full left-0 right-0 mt-1 bg-surface-2 border border-border rounded-lg shadow-xl overflow-hidden max-h-72 overflow-y-auto">
                {searchResults.map((result, i) => (
                  <button key={`${result.id}-${i}`} onClick={() => navigateToResult(result)} onMouseEnter={() => setSelectedIdx(i)}
                    className={`w-full text-left px-3 py-2.5 flex items-start gap-2.5 transition-colors ${i === selectedIdx ? 'bg-glass-strong' : 'hover:bg-row-hover'}`}>
                    {result.type === 'heading' ? (
                    ...
                    {i === selectedIdx && <kbd className="text-[9px] px-1 py-0.5 rounded bg-glass-strong text-text-tertiary font-mono self-center shrink-0">Enter</kbd>}
                  </button>
                ))}
              </div>
            )}
  ...
  return (
    <div>
      <button onClick={() => setOpen(!open)}
        className="w-full flex items-center gap-1 py-[3px] pr-2 hover:bg-glass transition-colors group/f"
...
function FileNode({ node, depth, selected, onSelect }: { node: TreeNode; depth: number; selected: boolean; onSelect: (path: string) => void }) {
  ...
  return (
    <button onClick={() => onSelect(node.path)}
      className={`w-full flex items-center gap-1.5 py-[3px] pr-2 transition-colors ${selected ? 'bg-accent-400/10 text-accent-300' : 'hover:bg-glass text-text-primary'}`}
