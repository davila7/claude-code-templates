  ...
  if (loading) {
    return (
      <div className="px-6 py-20 flex flex-col items-center gap-3">
        ...
        <span className="text-[13px] text-text-tertiary">Loading trending data...</span>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="px-6 pb-4">
        <div className="flex items-center gap-4 overflow-x-auto">
          <span className="text-[11px] text-text-tertiary uppercase tracking-wider shrink-0">Top Countries</span>
          {data.topCountries.map((c) => (
            <div key={c.code} className="flex items-center gap-1.5 shrink-0">
              ...
              <span className="text-[11px] text-text-tertiary tabular-nums">{c.percentage}%</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 px-6 py-3">
        ...
        <select
          ...
          className="ml-auto bg-input-bg border-none rounded-lg text-[12px] text-text-secondary px-2.5 py-1.5 outline-none cursor-pointer"
        >
      ...
      <div className="px-6 pb-8">
        <div className="bg-surface-2 border border-border rounded-lg overflow-hidden">
          {/* Table header */}
          <div className="grid grid-cols-[40px_1fr_100px_100px_100px_100px] gap-2 px-4 py-2 border-b border-border text-[11px] text-text-tertiary uppercase tracking-wider">
            <span>#</span>
            <span>Component</span>
          ...
          {items.map((item, idx) => {
            ...
            return (
              <div
                key={item.id}
                className="grid grid-cols-[40px_1fr_100px_100px_100px_100px] gap-2 px-4 py-2.5 border-b border-border last:border-b-0 hover:bg-row-hover transition-colors group"
              >
                {/* Rank */}
                <span className="text-[12px] text-text-tertiary tabular-nums">{idx + 1}</span>
                ...
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <span className="text-[13px] text-text-primary truncate block">{formatName(item.name)}</span>
                    <span className="text-[10px] text-text-tertiary">{item.category}</span>
                  </div>
                </div>

                {/* Stats */}
                <span className={`text-[12px] text-right tabular-nums self-center ${item.downloadsToday > 0 ? 'text-emerald-400' : 'text-text-tertiary'}`}>
                  {item.downloadsToday > 0 ? `+${item.downloadsToday}` : '0'}
                </span>
                <span className="text-[12px] text-right tabular-nums self-center text-text-secondary">
                  {item.downloadsWeek.toLocaleString()}
                </span>
                <span className="text-[12px] text-right tabular-nums self-center text-text-secondary">
                  {item.downloadsMonth.toLocaleString()}
                </span>
                <span className="text-[12px] text-right tabular-nums self-center text-text-primary">
