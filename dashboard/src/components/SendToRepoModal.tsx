...
export default function SendToRepoModal({ items, collectionName, onClose }: Props) {
  ...
  return (
    ...
    >
      <div className="w-full max-w-lg mx-4 bg-surface-2 border border-border rounded-xl shadow-2xl overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <div className="flex items-center gap-2.5">
            ...
            <h3 className="text-sm font-semibold text-text-primary">Send to GitHub</h3>
          </div>
          <button
            onClick={onClose}
            className="p-1 rounded hover:bg-glass-hover text-[--color-text-tertiary] hover:text-text-primary transition-colors"
          >
        ...
        <div className="px-5 py-4">
          ...
          {step === 'connect' && (
            <div className="text-center py-6">
              ...
              <button
                onClick={startOAuth}
                className="inline-flex items-center gap-2 px-5 py-2.5 bg-inverse-bg text-inverse-text rounded-lg text-sm font-medium hover:opacity-90 transition-colors"
              >
            ...
            <div>
              <div className="mb-3">
                <input
                  ...
                  className="w-full bg-input-bg border border-border rounded-lg px-3 py-2 text-sm text-text-primary placeholder:text-text-tertiary outline-none focus:border-border-hover transition-colors"
                  autoFocus
                />
              </div>
                  ...
                  {filteredRepos.map((repo) => (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="text-[13px] text-text-primary truncate">{repo.full_name}</div>
                        {repo.description && (
              ...
              {selectedRepo && (
                <div className="mt-4 pt-3 border-t border-border flex items-center justify-between">
                  <div className="text-[12px] text-[--color-text-tertiary]">
                    {items.length} component{items.length !== 1 ? 's' : ''} → <span className="text-text-primary">{selectedRepo.full_name}</span>
                  </div>
                  <button
          ...
          {step === 'creating' && (
            <div className="text-center py-8">
              <div className="w-6 h-6 border-2 border-border border-t-text-primary rounded-full animate-spin mx-auto mb-3" />
          ...
          {step === 'done' && prResult && (
            <div className="text-center py-6">
              ...
              <p className="text-sm font-medium text-text-primary mb-1">Pull Request Created!</p>
          ...
          {step === 'error' && (
            <div className="text-center py-6">
              ...
              <button
                ...
                className="px-4 py-2 bg-[--color-surface-3] hover:bg-[--color-surface-4] rounded-lg text-sm text-[--color-text-secondary] hover:text-text-primary transition-colors"
              >
                Try Again
              </button>
            </div>
